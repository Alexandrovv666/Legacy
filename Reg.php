<?php
    ...
    do {
        ...
        $x                 = rand(1, $Max_X_map);
        $y                 = rand(1, $Max_Y_map);
        $count_res_castle  = mysql_num_rows(mysql_query('SELECT x FROM `castle` WHERE `x` ="' . $x . '" AND `y` ="' . $y . '"'));
...
metka1:
    ...
    mysql_query('INSERT INTO `castle` (`id`, `x`, `y`, `name`, `gold`, `tree`, `stone`) VALUES ("'.F_Get_ID($login).'" , ' . $x . ',' . $y . ',"Безымянный замок",2000,1000,600)');
