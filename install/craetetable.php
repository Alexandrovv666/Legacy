<?//PEAR style
    ...
    mysql_query("CREATE TABLE IF NOT EXISTS `haus` (
 `id` int(11) NOT NULL,
 `new` text NOT NULL,
 ...
 `asklad` int(11) NOT NULL) ENGINE=InnoDB DEFAULT CHARSET=utf8;");
    ...
    mysql_query("CREATE TABLE `map` (`id` int(11) NOT NULL,`x` int(11) NOT NULL, `y` int(11) NOT NULL) ENGINE=InnoDB  DEFAULT CHARSET=utf8 AUTO_INCREMENT=1;");
    ...
    $qwery .= " `name` text NOT NULL, `timegate` int(11) NOT NULL,";
    ...
    for ($x = 1; $x <= 10; $x++){
        ...
        for ($y = 1; $y <= 10; $y++){
            $qwery .= ' ADD COLUMN `c_'.($x).'_'.($y).'_n` text NOT NULL,';
            $qwery .= ' ADD COLUMN `c_'.($x).'_'.($y).'_1` int(11) NOT NULL,';
            $qwery .= ' ADD COLUMN `c_'.($x).'_'.($y).'_2` int(11) NOT NULL,';
            $qwery .= ' ADD COLUMN `c_'.($x).'_'.($y).'_3` int(11) NOT NULL,';
        }
    ...
    echo '<html><head><meta http-equiv=Refresh content="4; url=createhaus.php"></head></html>';
?>