<?php
    ...
    for ($i = 0; $i <= $Max_level_HAUS; $i++)
        if ($i == 0)
            mysql_query('INSERT INTO `haus`(`number`, `alt_room`, `new_room`, `name`, `opisanie`,`gold`, `stone`, `atree`) VALUES ("' . ($i + $DELTA) . '","","lesop' . ($i + 1) . '","Комната мастера по дереву","В этой комнате проживает мастер по дереву.", "' . (25 * 3 * ($i + 1)) . '","1","' . (6 * ($i + 1)) . '")');
        else
            mysql_query('INSERT INTO `haus`(`number`, `alt_room`, `new_room`, `name`, `opisanie`,`gold`, `stone`, `atree`) VALUES ("' . ($i + $DELTA) . '","lesop' . ($i) . '","lesop' . ($i + 1) . '","Комната мастера по дереву","В этой комнате проживает мастер по дереву.", "' . (25 * 3 * ($i + 1)) . '","' . (2 * ($i)) . '","' . (6 * ($i + 1)) . '")');
    $DELTA = $DELTA + $DELTA_X;
    for ($i = 0; $i <= $Max_level_HAUS; $i++)
        if ($i == 0)
            mysql_query('INSERT INTO `haus`(`number`, `alt_room`, `new_room`, `name`, `opisanie`,`gold`, `tree`, `astone`) VALUES ("' . ($i + $DELTA) . '","","kamen' . ($i + 1) . '","Комната мастера по камню","В этой комнате проживает мастер по камню.","' . (30) . '","5","' . ($i + 1) . '")');
        else
            mysql_query('INSERT INTO `haus`(`number`, `alt_room`, `new_room`, `name`, `opisanie`,`gold`, `tree`, `astone`) VALUES ("' . ($i + $DELTA) . '","kamen' . ($i) . '","kamen' . ($i + 1) . '","Комната мастера по камню","В этой комнате проживает мастер по камню.","' . (30 * 3 * ($i + 1)) . '","' . (5 * ($i) * 3) . '","' . (3 * ($i + 1)) . '")');
    $DELTA = $DELTA + $DELTA_X;
    for ($i = 0; $i <= $Max_level_HAUS; $i++)
        if ($i == 0)
